...
TaskEntity TaskEntity::createTaskEntity(const Task &task, IdGenerator &idGenerator, bool status) {
  return TaskEntity(task, idGenerator.generateId(), status, std::vector<std::shared_ptr<TaskEntity>>());
}
