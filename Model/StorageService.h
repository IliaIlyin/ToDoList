...
class StorageService {
 ...
 public:
  ...
  std::shared_ptr<TaskEntity> addSubTaskToParent(TaskEntity &parent, std::string taskName, time_t date = 0,
                          Task::Priority priority = Task::Priority::NONE, std::string label = "");
  std::shared_ptr<TaskEntity> addTask(Task &task);
  std::shared_ptr<TaskEntity> addSubTaskToParent(TaskEntity &parent, Task &task);
  std::shared_ptr<TaskEntity> addSubTaskToParent(TaskEntity &parent, TaskEntity &task_entity);
