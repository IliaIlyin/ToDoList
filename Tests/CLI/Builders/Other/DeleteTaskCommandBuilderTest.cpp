...
class DeleteTaskCommandBuilderTest : public testing::Test {

};

TEST_F(DeleteTaskCommandBuilderTest, shouldCreateDeleteTaskCommand) {
  ...
  DeleteTaskCommandBuilder builder(std::move(mock));
  auto res = builder.buildCommand(service);
  ASSERT_NE(res.get(),nullptr);
}
TEST_F(DeleteTaskCommandBuilderTest, shouldCreateCancelCommand) {
  std::unique_ptr<InputStateMachineMock> mock = std::make_unique<InputStateMachineMock>();
  InputContext context;
  context.setName("ELON");
  context.setDate(boost::gregorian::date{2000,11,11});
