...
class CompleteTaskCommandBuilderTest : public testing::Test {

};

TEST_F(CompleteTaskCommandBuilderTest, shouldCreateCompleteTaskCommand) {
  ...
  CompleteTaskCommandBuilder builder(std::move(mock));
  auto res = builder.buildCommand(service);
  ASSERT_NE(res.get(), nullptr);
}
TEST_F(CompleteTaskCommandBuilderTest, shouldCreateCancelCommand) {
  auto mock = std::make_unique<InputStateMachineMock>();
  InputContext context;
  context.setName("ELON");
  context.setDate(boost::gregorian::date{2000, 11, 11});
