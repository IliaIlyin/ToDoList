...
class AddTaskCommandBuilderTest : public testing::Test {

};

TEST_F(AddTaskCommandBuilderTest, shouldCreateAddTaskCommand) {
  ...
  AddTaskCommandBuilder builder(std::move(mock));
  auto res = builder.buildCommand(service);
  ASSERT_NE(res.get(), nullptr);
}
TEST_F(AddTaskCommandBuilderTest, shouldCreateCancelCommand) {
  auto mock = std::make_unique<InputStateMachineMock>();
  InputContext context;
  context.setName("ELON");
  context.setDate(boost::gregorian::date{2000, 11, 11});
